...
describe('CurrencyCalculatorComponent', () => {
  ...
  beforeEach(async () => {
    ...
    const mockCurrencyService = {
      getSupportedCurrencies: jasmine.createSpy('getSupportedCurrencies').and.returnValue(
        Promise.resolve(['AUD', 'CHF', 'EUR', 'USD'])
      ),
      getSupportedCurrenciesWithNames: jasmine.createSpy('getSupportedCurrenciesWithNames').and.returnValue(
        Promise.resolve({
  ...
  it('should handle invalid amount', async () => {
    ...
    expect(component.error).toBe('Please enter a valid amount greater than 0' as any);
  ...
  xit('should call currency service for different currencies', async () => {
    ...
    expect(component.result).toEqual(mockResponse as any);
  });
  ...
  xit('should handle service errors', async () => {
    ...
    expect(convertSpy).toHaveBeenCalledWith('CHF', 'EUR', 1);
    expect(component.error).toBe('Failed to fetch exchange rates. Please try again later.' as any);
  });
  ...
  xit('should handle currency loading errors and fallback', async () => {
    ...
    (currencyService.getSupportedCurrencies as jasmine.Spy)
      .and.returnValue(Promise.resolve(fallbackCurrencies));
  ...
  xit('should set currencies loading state correctly', async () => {
    ...
    (currencyService.getSupportedCurrenciesWithNames as jasmine.Spy)
      .and.returnValue(promise as Promise<any>);
